...
from data_utils.dataset import DocumentFormatter, DocumentDataset
...
def main(parser):
    ...
    doc_formatter = DocumentFormatter(vocab=vocab, tokenizer=tokenizer, maxlen = model_config.maxlen, model_dir=model_dir)

    # Train & Val Datasets
    cwd = Path.cwd()
    data_in = cwd / "data_in"
    train_data_dir = data_in #/ "test.txt"

    tr_clf_ds = DocumentDataset(train_data_dir=train_data_dir, model_dir=model_dir)
    tr_clf_ds.set_transform_fn(transform_source_fn=doc_formatter.transform_source_fn, transform_target_fn=doc_formatter.transform_target_fn)
    tr_clf_dl = DataLoader(tr_clf_ds, batch_size=model_config.batch_size, shuffle=True, num_workers=0, drop_last=False)

    # Model
    model = KobertSequenceFeatureExtractor(config=model_config, num_classes=len(tr_clf_ds.tag_to_index)) #len(tr_clf_ds.tag_to_index))
    ...
    for _epoch, _ in enumerate(train_iterator):
        epoch_iterator = tqdm(tr_clf_dl, desc="Iteration", position=0, leave=True) # , disable=args.local_rank not in [-1, 0]
        epoch = _epoch
        for step, batch in enumerate(epoch_iterator):
            model.train()
            x_input, token_type_ids, y_real = map(lambda elm: elm.to(device), batch)
            # print('input>>', x_input.size())

            logits = model(x_input.to(device), token_type_ids.to(device))
            # logits = model(x_input, token_type_ids).to(device)
            ...
            if (step + 1) % model_config.gradient_accumulation_steps == 0:
                ...
                # if step % 50 == 0:
                epoch_iterator.set_postfix({'epoch':epoch + 1, 'global_step': global_step, 'loss': tr_summary['loss'], 'acc': tr_summary['acc']})
